...
export default function ModelsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    list = [...list].sort((a, b) => {
      ...
      // price: sort by average price ascending
      ...
      <header className="border-b border-border">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-6">
            ...
            <Link href="/models" className="font-body text-sm text-accent-blue font-medium">
              Models
            </Link>
            <LocaleToggle />
            <ThemeToggle />
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="font-body text-txt-secondary">
            {isZh
              ? "浏览所有模型的能力评分与价格信息"
              : "Browse capability scores and pricing for all models"}
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-txt-muted font-body whitespace-nowrap">
              {isZh ? "排序:" : "Sort:"}
            </span>
            {(
              [
                { key: "score" as SortKey, label: isZh ? "综合分" : "Score" },
        ...
        <div className="mb-4">
          <span className="text-xs text-txt-muted font-mono">
            {filtered.length} {isZh ? "个模型" : "models"}
          </span>
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="paper-card-flat p-12 text-center">
            <p className="font-body text-txt-muted text-lg mb-2">
              {isZh ? "没有找到匹配的模型" : "No models found"}
            </p>
            <p className="font-body text-txt-muted text-sm">
              {isZh ? "尝试调整搜索或筛选条件" : "Try adjusting your search or filters"}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map((m) => {
              ...
              const avgPrice = (m.pricing.input_per_1m + m.pricing.output_per_1m) / 2
              ...
              return (
                <div key={m.slug} className="paper-card p-5 flex flex-col">
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    ...
                    {m.tags
                      .filter((tag) => tag !== "frontier")
                      .slice(0, 3)
                      .map((tag) => (
                        <span
                          key={tag}
                          className="text-[10px] px-1.5 py-0.5 border border-border text-txt-muted font-mono tracking-wider"
                        >
                          {tag}
                        </span>
                      ))}
                    ...
                    {categoryKeys.map((key) => {
                      ...
                      return (
                        <div key={key} className="flex items-center gap-1.5">
                          <span className="text-[10px] w-8 text-txt-muted font-mono">
                            {getCategoryLabel(key).slice(0, 2)}
                          </span>
                          ...
                          <span className="text-[10px] font-mono text-txt-muted w-5 text-right">
                  ...
                  <div className="mt-auto pt-3 border-t border-border">
                    <div className="flex items-end justify-between">
                      <div>
                        <div className="text-xs text-txt-muted mb-0.5">
                          {t("home.composite")}
                        </div>
                        <span className="font-mono text-3xl text-txt-primary">
                  ...
                  <div className="flex items-center gap-4 mt-4 pt-3 border-t border-border">
                    ...
                    <Link
                      href={`/compare?ids=${m.slug}`}
                      className="text-xs font-body text-txt-muted hover:text-txt-secondary transition-colors"
                    >
