...
import { getModelBySlug, getCategories, getProviders } from "@/lib/data"
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h1 className="font-heading text-4xl text-txt-primary mb-4">404</h1>
          <p className="text-txt-muted mb-6">Model not found</p>
          <Link href="/models" className="btn-primary px-6 py-2 inline-block">
            Back to Models
          </Link>
        </div>
      </div>
        ...
        <div className="paper-card p-6 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {model.is_open_source && (
                  <span className="text-xs px-2 py-0.5 border border-score-high text-score-high font-mono">
                    Open Source
                  </span>
                )}
                {model.is_reasoning_model && (
                  <span className="text-xs px-2 py-0.5 border border-score-mid text-score-mid font-mono">
                    Reasoning Model
                  </span>
                )}
                {model.tags.map((tag) => (
                  <span key={tag} className="text-xs px-2 py-0.5 border border-border text-txt-muted font-mono">
                    {tag}
                  </span>
                ))}
              </div>
            ...
            <div className="flex flex-col items-end gap-2">
              ...
              <Link
                href={`/compare?ids=${model.slug}`}
                className="btn-primary px-4 py-2 text-sm inline-block mt-2"
              >
                Compare with others →
              </Link>
            </div>
                ...
                {categoryKeys.map((key) => {
                  ...
                  return (
                    <div key={key} className="flex items-center gap-3">
                      ...
                      <div className="w-16 text-sm font-body text-txt-secondary">
                      ...
                      <div className="w-16 text-right">
                        <span className="text-xs text-txt-muted">
                          {cat.availableCount}/{cat.benchmarkCount}
                          {!cat.isReliable && " ⚠️"}
                        </span>
                      </div>
                    </div>
                      ...
                      {catDef.benchmarks.map((bm) => {
                        ...
                        return (
                          <div key={bm.key} className="flex items-center gap-3 text-sm">
                            <div className="w-40 text-txt-secondary font-body truncate" title={bm.label}>
                              {bm.label}
                            </div>
                                ...
                                <div className="flex items-center gap-2">
                                  <div className="flex-1 h-2 bg-border rounded-full overflow-hidden">
                                    <div
                                      ...
                                      style={{
                                        width: bm.max_score
                                          ? `${Math.min((raw / bm.max_score) * 100, 100)}%`
                                          : "50%",
                                        backgroundColor: color,
                                      }}
                                    />
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-mono text-txt-primary">
                    {model.pricing.confirmed ? "" : "~"}${model.pricing.input_per_1m.toFixed(2)}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-txt-muted">Output / 1M tokens</span>
                  <span className="font-mono text-txt-primary">
                    {model.pricing.confirmed ? "" : "~"}${model.pricing.output_per_1m.toFixed(2)}
                  </span>
                </div>
                <div className="border-t border-border pt-3">
                  ...
                  <p className="text-xs text-txt-muted mt-1">
                    {model.is_reasoning_model
                      ? "1K input + 5K output tokens (reasoning)"
                      : "1K input + 500 output tokens"}
                  </p>
                </div>
                {!model.pricing.confirmed && (
                  <p className="text-xs text-score-mid mt-2">~ = estimated price</p>
                )}
              </div>
            </div>
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-txt-primary font-mono">
                    {(model.context_window_input / 1000).toFixed(0)}K
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-txt-muted">Context (Output)</span>
                  <span className="text-txt-primary font-mono">
                    {(model.context_window_output / 1000).toFixed(0)}K
                  </span>
                </div>
                <div className="flex justify-between">
