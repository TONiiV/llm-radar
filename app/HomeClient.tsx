...
export default function HomeClient({ models, categories, providers }: Props) {
  ...
  return (
    <div className="min-h-screen">
                ...
                return (
                  <div key={m.slug} className="space-y-1">
                    <div className="flex items-center justify-between">
                      <span className="text-crt-green">{i + 1}. {m.name}</span>
                      <span className="text-crt-green">{score}/100</span>
                    </div>
                    <div className="w-full h-1.5 bg-white/10">
                      <div className="h-full bg-crt-green/70" style={{ width: `${score}%` }} />
                    </div>
                  </div>
                )
              })}
      ...
      <section className="max-w-6xl mx-auto px-4 pb-16">
        <div className="section-divider mb-8">
          <h2 className="font-heading text-2xl italic text-txt-primary">{t("home.sixDimensions")}</h2>
        </div>
              ...
              <div key={key} className="paper-card p-4 text-center">
                ...
                <ul className="space-y-1">
                  {cat.benchmarks.map((bm) => (
                    <li key={bm.key} className="font-mono text-sm text-txt-muted">{bm.label}</li>
                  ))}
                </ul>
              </div>
            )
